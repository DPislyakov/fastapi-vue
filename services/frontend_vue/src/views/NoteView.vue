<template>
    <div v-if="note">
      <p><strong>Название статьи:</strong> {{ note.title }}</p>
      <p><strong>Текст:</strong> {{ note.content }}</p>
      <p><strong>Автор:</strong> {{ note.author.username }}</p>
  
      <div v-if="user.id === note.author.id">
        <p><router-link :to="{name: 'EditNote', params:{id: note.id}}" class="btn btn-primary">Изменить</router-link></p>
